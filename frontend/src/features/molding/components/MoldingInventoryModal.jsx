  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4 py-10">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[800px] max-h-full flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Search */}
        <div className="px-4 sm:px-5 py-3 border-b border-whisper bg-white">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-warm-white border border-whisper rounded-lg pl-8 pr-3 py-2 text-[13px] focus:outline-none focus:border-orange-400"
              value={invSearch}
        ...
        <div className="flex-1 overflow-y-auto bg-warm-white/30 p-4">
          {groupedInventory.length === 0 ? (
            <div className="text-center py-10 text-warm-gray-400 text-[13px]">Không có phôi phù hợp trong kho.</div>
          ) : (
              ...
              {groupedInventory.map(group => {
                ...
                return (
                  <div key={group.batchId} className="bg-white border border-whisper rounded-lg overflow-hidden shadow-sm">
                    {/* Parent Row */}
                    <div
                      className="flex items-start sm:items-center gap-3 p-3 bg-gray-50/80 border-b border-whisper hover:bg-gray-100/80 transition cursor-pointer"
                      onClick={() => handleToggleModalBatchOpen(group.batchId)}
                    >
                      <div
                      ...
                      >
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${allSelected ? 'bg-orange-500 border-orange-500 text-white' : isIndeterminate ? 'bg-orange-400 border-orange-400 text-white' : 'border-gray-300 bg-white'}`}>
                        ...
                        {group.items.map(item => {
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-start sm:items-center gap-2 sm:gap-3 p-2.5 pl-8 sm:pl-10 cursor-pointer transition ${isSelected ? 'bg-orange-50' : 'hover:bg-warm-white/50'}`}
                            >
                              <div className={`w-4 h-4 mt-0.5 sm:mt-0 rounded border flex items-center justify-center shrink-0 ${isSelected ? 'bg-orange-500 border-orange-500 text-white' : 'border-gray-300 bg-white'}`}>
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50">
          <div className="text-[13px] text-gray-600 font-medium">
            Đã chọn: <span className="text-orange-600 font-bold">{selectedInputs.length}</span> phôi
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 text-sm font-semibold text-white bg-gray-800 rounded-lg hover:bg-gray-700 active:scale-[0.98] transition shadow-sm"
          >
            Đóng
          </button>
        </div>
